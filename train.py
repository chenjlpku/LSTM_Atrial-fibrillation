...
# 模型参数
parser.add_argument('--init_from',type=str,default=None,help='address of trained model')
...
parser.add_argument('--output_keep_prob',type=float,default=0.5,help='output keep prob')
...
def train(args):
    ...
    train_writer = tf.summary.FileWriter(logdir=os.path.join(args.log_path,timestamp+'-training'),graph=tf.get_default_graph())
    validation_writer = tf.summary.FileWriter(logdir=os.path.join(args.log_path,timestamp+'-validation'),graph=tf.get_default_graph())
    with open(os.path.join(args.save_path,'config.pkl'),'wb') as f:
        cPickle.dump(args,f)
    
    model = Model(args)
    init_op = tf.group(tf.global_variables_initializer(),tf.local_variables_initializer())
    saver = tf.train.Saver(tf.global_variables(),max_to_step = 5)
    ...
    with tf.Session() as sess:
        ...
        for e in range(args.num_epoches):
            data_loader.reset_batch_pointer() # 每一轮开始都要重新设置batch的起点
            state = np.zeros([args.batch_size,args.cell_size])
            
            for b in range(data_loader.num_batches):
                start = time.time()
                x,y = data_loader.next_batch()
                feed  = {model.input_data:x, model.targets:y, model.initial_state:state}
                summ, train_loss, state,_ = sess.run([model.merge,model.loss,model.final_state,model.train_op],feed)
                train_writer.add_summary(summ,e*data_loader.num_batches+b)
                end = time.time()
                
                print("{}/{} (epoch {}), train_loss={:.3f},time/batch = {:.3f}".format(e*data_loader.num_batches+b,args.nums_epoches*data_loader.num_batches,e,train_loss,end-start))
                # 保存模型
