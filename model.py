...
class Model():
    def __init__(self,args,training = True):
        ...
        self.initial_state = tf.placeholder(tf.float32,[None,args.cell_size])
        
        outputs, states = tf.nn.dynamic_rnn(self.cell,self.input,initial_state = self.initial_state,dtype = tf.float32)
        ...
        with tf.name_scope('final_training_ops'):
            with tf.name_scope('weight'):
                weight = tf.get_variable('weights',initializer=tf.random_normal([args.cell_size,args.num_classes],stddev = 0.01))
            with tf.name_scope('biases'):
                bias = tf.get_variable('biases',initializer=tf.constant(0.1,shape=[args.num_classes]))           
            hf = tf.transpose(outputs,[1,0,2])
            last = tf.gather(hf,int(hf.get_shape()[0])-1)
            with tf.name_scope('logits'):
                self.logits = tf.matmul(last,weight)+bias
            with tf.name_scope('Prop'):
                self.prop = tf.nn.softmax(self.logits)
        # 计算损失，accuracy, 和优化器
        with tf.name_scope('loss'):
            loss = -tf.reduce_mean(self.target*tf.log(self.prop))
            self.loss = loss
